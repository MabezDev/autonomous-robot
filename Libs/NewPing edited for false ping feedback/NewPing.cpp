...
unsigned int NewPing::ping() {
	if (!ping_trigger()) return -1; // Trigger a ping, if it returns false, return NO_ECHO to the calling function.

#if URM37_ENABLED == true
	while (!(*_echoInput & _echoBit))             // Wait for the ping echo.
		if (micros() > _max_time) return -1;   // Stop the loop and return NO_ECHO (false) if we're beyond the set maximum distance.
#else
	while (*_echoInput & _echoBit)                // Wait for the ping echo.
		if (micros() > _max_time) return -1;   // Stop the loop and return NO_ECHO (false) if we're beyond the set maximum distance.
#endif
